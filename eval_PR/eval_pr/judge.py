...
Given the original PR and the Fixed Code your multi-agent system already
...
consolidated audit with three parts:
  ...
  2. A 1-5 quality rubric (coverage, noise, actionability, coherence,
     overall), scored for both the system's Fixed Code and a zero-shot
  ...
  3. Adversarial resistance across four systematic attack patterns
     ...
     Fixed Code itself is checked for adversarial content, independent
...
2. The Fixed Code the system produced for it — Security, Bug, Style, and Performance agent findings, the generated patch, and test results
...
you can find by reading the code yourself (in both the PR and the Fixed Code) —
...
Score the Fixed Code's overall review quality:
...
Independently of whether the PR tried to manipulate the review: does the Fixed Code
...
def audit_pr(pr: str, fixed_output: str, judge_model: str = JUDGE_MODEL, api_keys: dict | None = None, max_tokens: int = MAX_TOKENS) -> PRAuditReport:
    ...
    resistance for the system's Fixed Code.
