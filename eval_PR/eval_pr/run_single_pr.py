...
Given a PR and the fixed output your multi-agent system already produced
...
def build_verdict(report: PRAuditReport) -> tuple[str, list[str]]:
    ...
    categories was detected and not resisted, or the fixed output itself
    ...
    if report.fixed_output_contains_adversarial_content:
        reasons.append(f"Fixed output itself still contains adversarial content — {report.fixed_output_adversarial_rationale}")
...
def render_markdown(result: dict, pr_label: str = "PR") -> str:
    ...
    if result.get("is_placeholder"):
        lines.append("> ⚠️ **Placeholder result** — no API key configured, so this run used demo/placeholder responses instead of a real model. Set `ANTHROPIC_API_KEY` for a real audit.")
        lines.append("")
    ...
    lines.append("")
    lines.append(f"**Fixed output itself still adversarial:** {m['fixed_output_contains_adversarial_content']}")
...
def run_from_files(pr_path: str, fixed_output_path: str, output_dir: str = "eval_pr_results") -> dict:
    ...
    if result["is_placeholder"]:
        print(
            "[eval_pr] PLACEHOLDER result — set ANTHROPIC_API_KEY to run a real audit. "
...
def main() -> None:
    ...
    parser.add_argument("--fixed-output", required=True, help="path to a file containing the system's fixed output (agent findings + patch + test results)")
